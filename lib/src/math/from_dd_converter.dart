...
class FromDDConverter {
  ...
  ///
  /// This method uses the Transverse Mercator projection formulas to perform
  /// the conversion.
  ///
  /// Args:
  ///   x: The easting value in meters.
  ///   y: The northing value in meters.
  ///   lambda0: The longitude of the central meridian for the UTM zone, in
  ///            radians.
  ///
  /// Returns:
  ///   A list containing the latitude in radians at index 0 and the longitude
  ///   in radians at index 1.
  static List<double> _mapXYToLatLon(double x, double y, double lambda0) {
    ...
    final m = _arcLengthOfMeridian(x / utmScaleFactor);
    final mu = m /
        (smA *
            (1.0 -
                smEccSquared / 4.0 -
                3.0 * smEccSquared * smEccSquared / 64.0 -
                5.0 * math.pow(smEccSquared, 3.0) / 256.0));
    final phi1Rad = mu +
        (3.0 * smEccSquared / 2.0 - 27.0 * smEccSquared * smEccSquared / 32.0) *
            math.sin(2.0 * mu) +
        (21.0 * smEccSquared * smEccSquared / 16.0 -
                55.0 * math.pow(smEccSquared, 3.0) / 32.0) *
            math.sin(4.0 * mu) +
        (151.0 * math.pow(smEccSquared, 3.0) / 96.0) * math.sin(6.0 * mu);
    // final phi1 = _radToDeg(phi1Rad);
    ...
    const g = smA *
        ...
        (math.pi / 180.0);
    // final c = (smEccSquared * smEccSquared) * math.pow(math.cos(phi1Rad),
    // 2.0);
    final t = math.tan(phi1Rad) * math.tan(phi1Rad);
    final eta2 =
        (smEccSquared * smEccSquared) * math.pow(math.cos(phi1Rad), 2.0);
    final term1 = t / (2.0 * g * math.pow(utmScaleFactor, 2.0));
    final term2 = (t / (24.0 * g * math.pow(utmScaleFactor, 4.0))) *
        (5.0 + 3.0 * t + eta2 - 9.0 * t * eta2);
    final term3 = (t / (720.0 * g * math.pow(utmScaleFactor, 6.0))) *
        (61.0 + 90.0 * t + 45.0 * t * t);
    final term4 = (t / (40320.0 * g * math.pow(utmScaleFactor, 8.0))) *
        (61.0 + 662.0 * t + 1320.0 * t * t + 720.0 * t * t * t);

    final phiRad = phi1Rad -
        (term1 - term2 + term3 - term4) *
            (1.0 / math.cos(phi1Rad)) *
            ((x / (utmScaleFactor * smA)) - (lambda0 * math.pi / 180.0));
    final lambdaRad = lambda0 +
        ((x / (utmScaleFactor * smA)) - (lambda0 * math.pi / 180.0)) *
            (1.0 / math.cos(phi1Rad));

    return [phiRad, lambdaRad];
  }

  /// Converts radians to degrees.
  ///
  /// Args:
  ///   rad: Radians to be converted to degrees.
  ///
  /// Returns:
  ///   The equivalent value in degrees.
  static double _radToDeg(double rad) => rad / math.pi * 180.0;
